...
echo '<thead>';
echo '<tr>';
echo '<th>idConsole</th>';
echo '<th class="d-none d-md-table-cell">Console</th>';
echo '<th>Delete</th>';
echo '</tr>';  
echo '</thead>';
...
foreach ($console as $c) {
  echo '<tr>';
  echo '<td>' . $c['idConsole'] . '</td>';
  echo '<td class="d-none d-md-table-cell">' . $c['Console'] . '</td>';

  echo '<td><form action="admin.php" method="post">';
  echo '<input type="hidden" name="idConsole" value="' . $c['idConsole'] . '">';
